...
Cheaper Inference is a gateway that serves models from several upstream providers through one OpenAI-compatible API.  Models are addressed by their maker's own name, without a vendor prefix.  Each model costs 15–60% less than the list price of its lab.
